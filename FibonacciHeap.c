...
// cut: Вырезает узел x из списка сыновей узла y и помещает его 
// в список корней кучи H. Обновляет минимальный элемент кучи H.
static void cut( FibHeap* H, FHNode* x, FHNode* y )
{
	// Удаляем x из дочернего списка y.
	if (x->right == x)
		// Если узел x был единственным сыном y.
		y = NULL;
	else {
		// Додитель x не должен укаузывать на бывшего сына.
		y->child = x->right;

		// Связыаем левого и правого брта узла x.
	...
	// Добавляем x в список корней кучи H.
	x->left  			= H->min->left;
	x->right 			= H->min;
	H->min->left->right = x;
	H->min->left 		= x;

	// Делаем узел x корневым.
	x->p = NULL;

	// Обновляем минимальный элемент кучи H.
	if (H->min->key > x->key)
		H->min = x;
}
...
// cascadingCut: Выполняет обработку меток узлов текущей ветки кучи H
// от корня до y в соответствии с правилами фибоныииевой кучи.
static void cascadingCut( FibHeap* H, FHNode* y )
{
	FHNode* z = y->p; // Указатель на родителя y.


	if (z != NULL) {
		// Если y не корневой узел, то:
		if (y->mark == FALSE)
			// Если узел y не был помечен, помечаем его;
			y->mark = TRUE;
		else {
			// Иначе вырезаем узел x из списка сыновей z ...
			cut(H, y, z);
			// ... и повторяем все эти действия уже для z.
			cascadingCut(H, z);
		}
	}
}
...
// decreaseKeyFH: Устанавливает начение ключа узла x в пирамиде H равным k.
// Значение k не должно превыщать текущего значение ключа x.
void decreaseKeyFH( FibHeap* H, FHNode* x, TBase k )
{
	// Если новое значение ключа превышает текущее значение ключа узла x,
	// то выполнение процедуры завершается.
	if (k > x->key) {} else {
		// Обновляем значени ключа узла x.
		x->key = k;

		FHNode* y = x->p; // указатель на родителя.

		// Если ключ узла x теперь оказался меньше, чем у родитетеля - y,
		// то перенсем его в список корней.
		if (x->key > y->key) {
			cut(H, x, y);
		}

		// Обновим минимальный узел.
		if (H->min->key > x->key)
			H->min = x;
	}
...
// deleteFH: Уддаляет узел x из кучи H.
void deleteFH( FibHeap* H, FHNode* x )
{
	// Установим принудительно указатель H.min на данный узел.
	H->min = x;

	// Вырежем узел x из списка сыновей его родителя, если таковой имеется.
	if (x->p != NULL)
		cut(H, x, x->p);

	// Выполним удаление минимального узла в куче.
	extractMinFM(H);
}