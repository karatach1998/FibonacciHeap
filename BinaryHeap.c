...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<
ДОБАВЛЯТЬ ЛИ ПОДДЕЖКУ ИЗМЕНЕНИЯ НЕВОЗРАСТАЮЩЕЙ/НЕУБЫВАЮЩЕЙ ПИРАМИДЫ
ПУТЕМ ЗАМЕНЫ МАКРОСОВ.

 В ОТЧЕТЕ ПРИЛАГАТЬ ИСХОДНЫЙ КОД ИЛИ КОД С ТЕСТАМИ И СЧЕТЧИКАМИ.
*///>>>>>>>>>>>>>>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<<<<<<<<<<
